...
from stock_data import (
    process_current_stock_data,
    get_stock_data,
    process_intraday_price_history,
)
    ...
    try:
        ...
        for symbol in stock_symbols:
            quote_data, symbol = get_stock_data(
                symbol, daily_data_needed=False, intraday_data_needed=False
            )
...
def update_intraday_price_history(stock_symbols, month):
    ...
    try:
        ...
        for symbol in stock_symbols:
            intraday_data, symbol = get_stock_data(
                symbol, month, daily_data_needed=False, current_data_needed=False
            )
            ...
            for timestamp in timestamps:
                processed_data = process_intraday_price_history(
                    intraday_data, symbol, timestamp
                )
...
def update_all_portfolios():
    try:
        ...
        for user_id in user_ids:
            cursor.execute(
                """SELECT DISTINCT stock_symbol FROM transactions WHERE user_id = ?""",
                (user_id,),
            )
            ...
            for stock_symbol in stock_symbols:
                ...
                cursor.execute(
                    """SELECT average_price, shares, total_cost_basis
                                FROM portfolios
    ...
    try:
        ...
        if transaction_type == "BUY":
            if portfolio:
                ...
                avg_price = (
                    (portfolio[0] * portfolio[1]) + (shares * price_per_share)
                ) / new_total_shares
            ...
            if not portfolio:
                raise ValueError(
                    f"You first have to buy shares of {stock_symbol}, before selling."
                )
            ...
            if shares > current_shares:
                raise ValueError(
                    f"You cannot sell more shares of {stock_symbol} than you have"
                )
...
if __name__ == "__main__":
    ...
    def update_stocks():
        ...
        for symbol in test_symbols:
            cursor.execute(
                "SELECT * FROM stocks_current WHERE stock_symbol = ?", (symbol,)
            )
            print(cursor.fetchone())
        connection.close()
    ...
    scheduler.add_job(
        func=update_stocks, trigger="interval", seconds=60, misfire_grace_time=60
    )
